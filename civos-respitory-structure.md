# Repository structure

This file retains its original path so existing links continue to work.

```text
civos/               Python entry point, validator, ledger and report
examples/            Fictional runnable JSON input
tests/              Model, ledger and command line tests
docs/                Record format and pilot protocol
articles/            English and Swedish manuscripts
.github/workflows/   Automated checks
README.md            Start here
technical-overview.md
philosophical-basis.md
climate-knowledge-integration.md
CONTRIBUTING.md
CODE_OF_CONDUCT.md
LICENSE
```

Local databases, exports and generated reports belong in the ignored `work/` directory. The files `thedarkmirror` and `Join the CivOS experiment` retain their historical names and contain the failure analysis and contribution invitation.
